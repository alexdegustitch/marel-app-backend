...
-- V1 baseline
-- =============================================================================
-- The starting point for Flyway. Before this, 110 SQL scripts under
-- src/main/resources/sql/ (now src/main/resources/sql/archive/) were applied
-- by hand with psql and nothing recorded which database had which of them.
--
-- This file is the schema and reference data those 110 scripts produce when
-- applied in order, dumped from a database built exactly the way the
-- integration suite built its test schema: the old baseline snapshot, then
-- every post-2026-07-21 script in filename order. That process is exercised
-- by every test run and just proved itself again immediately before this file
-- was generated.
--
-- Adopted with no production database to reconcile against — nothing had
-- been deployed yet, so there is no "does this match what's live" question to
-- answer and no baselineVersion to set. A project that already has a running
-- database needs to confirm this file matches it before trusting it; see
-- docs/flyway-adoption-plan.md for what that check looks like.
--
-- WHY INSERT, NOT COPY: pg_dump defaults to `COPY ... FROM stdin`, which is a
-- psql/libpq wire protocol feature, not standalone SQL - a JDBC Statement
-- (what Flyway executes scripts through) cannot run it. `--inserts` was used
-- so this file is plain SQL that runs the same way any other migration does.
--
-- WHAT CARRIES DATA: business tables are empty - no employees, work logs or
-- payroll runs existed anywhere this was built. What has rows is reference
-- and catalogue data the migrations themselves established: audit_actions
-- and audit_tables (audit_trigger_fn resolves both by name and fails a NOT
-- NULL check without them), the compensation scheme catalogue, payroll value
-- and work-code-category definitions, and one real department ("Komercijala")
-- that commercial staff need to exist. audit_logs carries the 17 entries
-- those same migrations wrote while making that data change - kept rather
-- than cleared, because rewriting the trail to look like nothing happened
-- would be the dishonest choice for a table whose whole job is the trail.
--
-- HOW TO REGENERATE, if this ever needs redoing before there is a production
-- database to baseline against instead:
--   1. docker run -d -e POSTGRES_PASSWORD=x -e POSTGRES_DB=ref -P postgres:18-alpine
--   2. Apply src/test/resources/db/baseline-schema.sql, then
--      src/test/resources/db/reference-data.sql, then every file in
--      src/main/resources/sql/archive/ from 2026-07-21- onward, in filename
--      order - see AbstractIntegrationTest.migrationScripts() for the exact rule.
--   3. pg_dump -U postgres -d ref --no-owner --no-privileges --no-comments --inserts
--   4. Strip the \restrict / \unrestrict lines pg_dump 18 adds - psql meta-commands,
--      not SQL, and Flyway's JDBC executor cannot parse them.
-- =============================================================================
...
--
-- Name: compensation_scheme_activation_needs_full_matrix(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.compensation_scheme_activation_needs_full_matrix() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_missing TEXT;
...
--
-- Name: employment_period_sync_employee_fn(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.employment_period_sync_employee_fn() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
--
-- Name: payroll_run_item_handovers_are_append_only(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.payroll_run_item_handovers_are_append_only() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    RAISE EXCEPTION
...
--
-- Name: payroll_time_adjustment_require_reason(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.payroll_time_adjustment_require_reason() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_require BOOLEAN;
...
--
-- Name: work_category_period_sync_employee_fn(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.work_category_period_sync_employee_fn() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_employee_id BIGINT := COALESCE(NEW.employee_id, OLD.employee_id);
...
--
-- Name: compensation_schemes; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.compensation_schemes (
    id bigint NOT NULL,
    code character varying(60) NOT NULL,
    name character varying(150) NOT NULL,
    allow_unmapped_categories boolean DEFAULT true NOT NULL,
    note text,
    is_active boolean DEFAULT true NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone,
    archived_at timestamp with time zone,
    allows_performance_bonus boolean DEFAULT true NOT NULL,
    CONSTRAINT chk_compensation_schemes_code_not_empty CHECK ((length(TRIM(BOTH FROM code)) > 0)),
    CONSTRAINT chk_compensation_schemes_name_not_empty CHECK ((length(TRIM(BOTH FROM name)) > 0)),
    CONSTRAINT chk_compensation_schemes_no_reactivate CHECK ((NOT ((archived_at IS NOT NULL) AND (is_active = true))))
);


--
-- Name: compensation_schemes_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.compensation_schemes ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.compensation_schemes_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.daily_reports (
    ...
    total_verified_minutes numeric(38,4),
    total_pl_minutes integer,
    total_plb_minutes integer,
    was_probation boolean DEFAULT false NOT NULL
);
...
--
-- Name: department_head_periods; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.department_head_periods (
    id bigint NOT NULL,
...
--
-- Name: department_head_periods_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.department_head_periods_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: department_head_periods_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.department_head_periods_id_seq OWNED BY public.department_head_periods.id;
...
--
-- Name: employee_compensation_scheme_history; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.employee_compensation_scheme_history (
    id bigint NOT NULL,
    employee_id bigint NOT NULL,
    compensation_scheme_id bigint CONSTRAINT employee_compensation_scheme_hi_compensation_scheme_id_not_null NOT NULL,
    valid_from date NOT NULL,
    valid_until date,
    note text,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone,
    archived_at timestamp with time zone,
    CONSTRAINT chk_ecsh_validity CHECK (((valid_until IS NULL) OR (valid_until >= valid_from)))
);


--
-- Name: employee_compensation_scheme_history_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.employee_compensation_scheme_history ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.employee_compensation_scheme_history_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: employee_employment_periods; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.employee_employment_periods (
    ...
    ended_on date,
    norm_grace_days integer DEFAULT 0 NOT NULL,
...
--
-- Name: employee_employment_periods_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.employee_employment_periods ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.employee_employment_periods_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: employee_payroll_value_definitions; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.employee_payroll_value_definitions (
    id bigint NOT NULL,
    ...
    updated_at timestamp with time zone,
    archived_at timestamp with time zone,
    CONSTRAINT chk_epvd_code CHECK ((length(TRIM(BOTH FROM code)) > 0)),
    CONSTRAINT chk_epvd_name CHECK ((length(TRIM(BOTH FROM name)) > 0)),
    CONSTRAINT chk_epvd_no_reactivate CHECK ((NOT ((archived_at IS NOT NULL) AND (is_active = true)))),
    CONSTRAINT chk_epvd_value_type CHECK (((value_type)::text = ANY ((ARRAY['NUMERIC'::character varying, 'BOOLEAN'::character varying, 'TEXT'::character varying])::text[])))
);


--
-- Name: employee_payroll_value_definitions_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.employee_payroll_value_definitions ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.employee_payroll_value_definitions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: employee_payroll_value_history; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.employee_payroll_value_history (
    id bigint NOT NULL,
    employee_id bigint NOT NULL,
    ...
    updated_at timestamp with time zone,
    archived_at timestamp with time zone,
    CONSTRAINT chk_epvh_validity CHECK (((valid_until IS NULL) OR (valid_until >= valid_from))),
    CONSTRAINT chk_epvh_value_matches_type CHECK (((((value_type)::text = 'NUMERIC'::text) AND (numeric_value IS NOT NULL) AND (boolean_value IS NULL) AND (text_value IS NULL)) OR (((value_type)::text = 'BOOLEAN'::text) AND (boolean_value IS NOT NULL) AND (numeric_value IS NULL) AND (text_value IS NULL)) OR (((value_type)::text = 'TEXT'::text) AND (text_value IS NOT NULL) AND (length(TRIM(BOTH FROM text_value)) > 0) AND (numeric_value IS NULL) AND (boolean_value IS NULL))))
);


--
-- Name: employee_payroll_value_history_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.employee_payroll_value_history ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.employee_payroll_value_history_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: employee_work_category_periods; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.employee_work_category_periods (
    id bigint NOT NULL,
    employee_id bigint NOT NULL,
    work_code_category_id bigint NOT NULL,
    valid_from date NOT NULL,
...
--
-- Name: employee_work_category_periods_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.employee_work_category_periods_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: employee_work_category_periods_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.employee_work_category_periods_id_seq OWNED BY public.employee_work_category_periods.id;
...
CREATE TABLE public.employees (
    ...
    preferred_locale character varying(35) DEFAULT 'sr-Latn'::character varying NOT NULL,
    ...
    CONSTRAINT chk_employees_bank_account CHECK (((bank_account IS NULL) OR (length(TRIM(BOTH FROM bank_account)) > 0))),
    CONSTRAINT chk_employees_email CHECK (((email IS NULL) OR ((length(TRIM(BOTH FROM email)) > 2) AND (POSITION(('@'::text) IN (email)) > 1)))),
    ...
    CONSTRAINT chk_employees_first_name CHECK ((length(TRIM(BOTH FROM first_name)) > 0)),
    CONSTRAINT chk_employees_hourly_rate_positive CHECK (((hourly_rate IS NULL) OR (hourly_rate >= (0)::numeric))),
    CONSTRAINT chk_employees_jmbg CHECK (((jmbg IS NULL) OR ((jmbg)::text ~ '^[0-9]{13}$'::text))),
    CONSTRAINT chk_employees_last_name CHECK ((length(TRIM(BOTH FROM last_name)) > 0)),
    CONSTRAINT chk_employees_preferred_locale CHECK (((preferred_locale)::text = ANY ((ARRAY['sr-Latn'::character varying, 'en'::character varying])::text[]))),
...
--
-- Name: operation_norm_versions; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.operation_norm_versions (
    id bigint NOT NULL,
    operation_id bigint NOT NULL,
    min_norm integer,
    max_norm integer,
    ...
    archived_at timestamp with time zone,
    CONSTRAINT chk_operation_norm_versions_norm_range CHECK (((min_norm IS NULL) OR (max_norm IS NULL) OR ((min_norm > 0) AND (max_norm > 0) AND (min_norm <= max_norm)))),
    CONSTRAINT chk_operation_norm_versions_verified_pair CHECK (((verified_at IS NULL) = (verified_by IS NULL)))
);


--
-- Name: operation_norm_versions_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.operation_norm_versions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: operation_norm_versions_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.operation_norm_versions_id_seq OWNED BY public.operation_norm_versions.id;
...
CREATE TABLE public.payroll_adjustment_categories (
    ...
    editable_input character varying(20) DEFAULT 'NONE'::character varying NOT NULL,
    allow_total_override boolean DEFAULT false NOT NULL,
    show_when_zero boolean DEFAULT true NOT NULL,
    required_manual_input boolean DEFAULT false NOT NULL,
    CONSTRAINT chk_pac_editable_input CHECK (((editable_input)::text = ANY ((ARRAY['NONE'::character varying, 'AMOUNT'::character varying, 'UNIT_AMOUNT'::character varying, 'QUANTITY'::character varying, 'CORRECTION'::character varying])::text[]))),
...
--
-- Name: payroll_adjustment_category_scheme_rules; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.payroll_adjustment_category_scheme_rules (
    id bigint NOT NULL,
    compensation_scheme_id bigint CONSTRAINT payroll_adjustment_category_sch_compensation_scheme_id_not_null NOT NULL,
    payroll_adjustment_category_id bigint CONSTRAINT payroll_adjustment_categor_payroll_adjustment_categor_not_null1 NOT NULL,
    ...
    updated_at timestamp with time zone,
    archived_at timestamp with time zone,
    calculation_mode character varying(20) DEFAULT 'INHERIT'::character varying CONSTRAINT payroll_adjustment_category_scheme_ru_calculation_mode_not_null NOT NULL,
    visible_in_ui boolean,
    visible_in_pdf boolean,
    show_when_zero boolean,
    editable_input character varying(20),
    allow_total_override boolean,
    required_manual_input boolean,
    parameters_override jsonb DEFAULT '{}'::jsonb CONSTRAINT payroll_adjustment_category_scheme_parameters_override_not_null NOT NULL,
    CONSTRAINT chk_pacsr_calculation_mode CHECK (((calculation_mode)::text = ANY ((ARRAY['INHERIT'::character varying, 'ZERO'::character varying, 'MANUAL'::character varying])::text[]))),
    CONSTRAINT chk_pacsr_editable_input CHECK (((editable_input IS NULL) OR ((editable_input)::text = ANY ((ARRAY['NONE'::character varying, 'AMOUNT'::character varying, 'UNIT_AMOUNT'::character varying, 'QUANTITY'::character varying, 'CORRECTION'::character varying])::text[])))),
    CONSTRAINT chk_pacsr_excluded_is_inert CHECK (((is_allowed = true) OR ((calculation_mode)::text = ANY ((ARRAY['INHERIT'::character varying, 'ZERO'::character varying])::text[])))),
    CONSTRAINT chk_pacsr_no_reactivate CHECK ((NOT ((archived_at IS NOT NULL) AND (is_active = true)))),
    CONSTRAINT chk_pacsr_validity CHECK (((valid_until IS NULL) OR (valid_until >= valid_from)))
);


--
-- Name: payroll_adjustment_category_scheme_rules_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.payroll_adjustment_category_scheme_rules ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.payroll_adjustment_category_scheme_rules_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: payroll_adjustment_category_translations; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.payroll_adjustment_category_translations (
    id bigint NOT NULL,
    payroll_adjustment_category_id bigint CONSTRAINT payroll_adjustment_category_payroll_adjustment_categor_not_null NOT NULL,
    locale character varying(35) NOT NULL,
    name character varying(255) NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone,
    CONSTRAINT chk_pact_locale_not_empty CHECK ((length(TRIM(BOTH FROM locale)) > 0)),
    CONSTRAINT chk_pact_name_not_empty CHECK ((length(TRIM(BOTH FROM name)) > 0))
);


--
-- Name: payroll_adjustment_category_translations_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.payroll_adjustment_category_translations ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.payroll_adjustment_category_translations_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.payroll_adjustments (
    ...
    correction_amount numeric(38,2) DEFAULT 0 NOT NULL,
    override_reason text,
    has_manual_input boolean DEFAULT false NOT NULL,
    ...
    section_code_snapshot character varying(50),
    impact_code_snapshot character varying(40),
    sort_order_snapshot integer,
    visible_in_ui_snapshot boolean,
    visible_in_pdf_snapshot boolean,
    show_when_zero_snapshot boolean,
    system_correction_amount numeric(38,2) DEFAULT 0 NOT NULL,
    CONSTRAINT chk_pa_override_reason CHECK (((is_overridden = false) OR ((override_reason IS NOT NULL) AND (length(TRIM(BOTH FROM override_reason)) > 0)))),
    CONSTRAINT chk_pa_status CHECK (((status)::text = ANY ((ARRAY['CALCULATED'::character varying, 'PENDING_INPUT'::character varying, 'MANUAL'::character varying, 'OVERRIDDEN'::character varying, 'ERROR'::character varying])::text[]))),
...
--
-- Name: payroll_field_access; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.payroll_field_access (
    id bigint NOT NULL,
    ...
    CONSTRAINT chk_pfa_edit_requires_view CHECK (((NOT can_edit) OR can_view)),
    CONSTRAINT chk_pfa_not_payroll_role CHECK ((lower((role_name)::text) <> ALL (ARRAY['admin'::text, 'developer'::text])))
);


--
-- Name: payroll_field_access_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.payroll_field_access ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.payroll_field_access_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: payroll_run_item_handovers; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.payroll_run_item_handovers (
    id bigint NOT NULL,
    payroll_run_item_id bigint NOT NULL,
    event character varying(255) NOT NULL,
    actor_id bigint,
    occurred_at timestamp with time zone DEFAULT now() NOT NULL,
    status_before character varying(255) NOT NULL,
    status_after character varying(255) NOT NULL,
    total_net_earnings numeric(38,2),
    net_payable_amount numeric(38,2),
    note text,
    payload jsonb DEFAULT '{}'::jsonb NOT NULL,
    CONSTRAINT chk_prih_event CHECK (((event)::text = ANY ((ARRAY['SUBMITTED'::character varying, 'RETURNED'::character varying])::text[])))
);


--
-- Name: payroll_run_item_handovers_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.payroll_run_item_handovers ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.payroll_run_item_handovers_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: payroll_time_adjustment_categories; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.payroll_time_adjustment_categories (
    id bigint NOT NULL,
    code character varying(100) NOT NULL,
    name character varying(150) NOT NULL,
    description text,
    impact_code character varying(40) DEFAULT 'PAYABLE_MINUTES'::character varying NOT NULL,
    calculation_key character varying(100) DEFAULT 'MANUAL'::character varying NOT NULL,
    allow_negative boolean DEFAULT true NOT NULL,
    allow_positive boolean DEFAULT true NOT NULL,
    ...
    updated_at timestamp with time zone,
    archived_at timestamp with time zone,
    CONSTRAINT chk_ptac_code CHECK ((length(TRIM(BOTH FROM code)) > 0)),
    CONSTRAINT chk_ptac_direction CHECK ((allow_negative OR allow_positive)),
    CONSTRAINT chk_ptac_impact_code CHECK (((impact_code)::text = 'PAYABLE_MINUTES'::text)),
    CONSTRAINT chk_ptac_name CHECK ((length(TRIM(BOTH FROM name)) > 0)),
    CONSTRAINT chk_ptac_no_reactivate CHECK ((NOT ((archived_at IS NOT NULL) AND (is_active = true))))
);


--
-- Name: payroll_time_adjustment_categories_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.payroll_time_adjustment_categories ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.payroll_time_adjustment_categories_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: payroll_time_adjustments; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.payroll_time_adjustments (
    id bigint NOT NULL,
    payroll_run_item_id bigint NOT NULL,
    payroll_time_adjustment_category_id bigint CONSTRAINT payroll_time_adjustments_payroll_time_adjustment_categ_not_null NOT NULL,
    system_minutes integer DEFAULT 0 NOT NULL,
    ...
    CONSTRAINT chk_pta_no_reactivate CHECK ((NOT ((archived_at IS NOT NULL) AND (is_applied = true))))
);


--
-- Name: payroll_time_adjustments_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.payroll_time_adjustments ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.payroll_time_adjustments_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.work_code_categories (
    ...
    allows_parallel_work boolean DEFAULT false NOT NULL,
    is_base_operation boolean DEFAULT true NOT NULL,
...
--
-- Name: work_code_category_mapping_types; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.work_code_category_mapping_types (
    id bigint NOT NULL,
    code character varying(100) NOT NULL,
    name character varying(150) NOT NULL,
    applies_during_probation boolean DEFAULT true CONSTRAINT work_code_category_mapping_ty_applies_during_probation_not_null NOT NULL,
    is_active boolean DEFAULT true NOT NULL,
    note text,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone,
    CONSTRAINT chk_wcmt_code_not_empty CHECK ((length(TRIM(BOTH FROM code)) > 0)),
    CONSTRAINT chk_wcmt_name_not_empty CHECK ((length(TRIM(BOTH FROM name)) > 0))
);


--
-- Name: work_code_category_mapping_types_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.work_code_category_mapping_types ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.work_code_category_mapping_types_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: work_code_category_scheme_rules; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.work_code_category_scheme_rules (
    id bigint NOT NULL,
    ...
    CONSTRAINT chk_wccsr_coefficient_positive CHECK (((coefficient_override IS NULL) OR (coefficient_override > (0)::numeric))),
    CONSTRAINT chk_wccsr_no_reactivate CHECK ((NOT ((archived_at IS NOT NULL) AND (is_active = true)))),
    CONSTRAINT chk_wccsr_validity CHECK (((valid_until IS NULL) OR (valid_until >= valid_from)))
);


--
-- Name: work_code_category_scheme_rules_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.work_code_category_scheme_rules ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.work_code_category_scheme_rules_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: work_code_category_translations; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.work_code_category_translations (
    id bigint NOT NULL,
    work_code_category_id bigint NOT NULL,
    locale character varying(35) NOT NULL,
    name character varying(255) NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone,
    CONSTRAINT chk_wcct_locale_not_empty CHECK ((length(TRIM(BOTH FROM locale)) > 0)),
    CONSTRAINT chk_wcct_name_not_empty CHECK ((length(TRIM(BOTH FROM name)) > 0))
);


--
-- Name: work_code_category_translations_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.work_code_category_translations ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.work_code_category_translations_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
END) STORED,
    ...
    compensation_scheme_id bigint,
    scheme_effective_work_code_category_id bigint,
    work_code_category_scheme_rule_id bigint,
...
--
-- Name: department_head_periods id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.department_head_periods ALTER COLUMN id SET DEFAULT nextval('public.department_head_periods_id_seq'::regclass);


--
-- Name: employee_work_category_periods id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_work_category_periods ALTER COLUMN id SET DEFAULT nextval('public.employee_work_category_periods_id_seq'::regclass);


--
-- Name: operation_norm_versions id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.operation_norm_versions ALTER COLUMN id SET DEFAULT nextval('public.operation_norm_versions_id_seq'::regclass);
...
--
-- Data for Name: absence_compensations; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: absence_records; Type: TABLE DATA; Schema: public; Owner: -
--
...
INSERT INTO public.audit_actions VALUES (1, 'insert');
INSERT INTO public.audit_actions VALUES (2, 'update');
INSERT INTO public.audit_actions VALUES (3, 'delete');
INSERT INTO public.audit_actions VALUES (4, 'login');
INSERT INTO public.audit_actions VALUES (5, 'logout');
INSERT INTO public.audit_actions VALUES (6, 'system');
...
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (1, NULL, 1, '2026-08-18 17:03:40.374467+00', 26, 1, '{"id": 1, "note": "Common effective category for the FOREIGN_FIXED_COEFFICIENT scheme. Never selected directly on a work log: it is resolved from a source category (J, D, ...) by work_code_category_scheme_rules. The source category is always preserved.", "type": "WORK", "is_paid": true, "is_active": true, "valid_from": "2026-08-01", "archived_at": null, "category_no": "FOREIGN_ALL_SHIFTS", "hourly_rate": null, "valid_until": null, "affects_norm": true, "affects_bonus": true, "base_category": true, "category_name": "I, II i III smena", "display_order": 1, "norm_multiplier": 1, "fixed_hourly_rate": false, "allows_parallel_work": false, "affects_meal_allowance": true}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (2, NULL, 1, '2026-08-18 17:03:40.528183+00', 26, 2, '{"category_no": {"new": "S", "old": "FOREIGN_ALL_SHIFTS"}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (3, NULL, 1, '2026-08-18 17:03:40.528183+00', 40, 1, '{"id": 1, "note": "Fixed coefficient: every shift and every trade is worth the same under this scheme.", "is_active": true, "is_allowed": true, "valid_from": "2026-08-01", "archived_at": null, "valid_until": null, "source_category_id": 1, "coefficient_override": 1.00, "effective_category_id": 1, "compensation_scheme_id": 2}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (4, NULL, 2, '2026-08-18 17:03:40.563776+00', 38, 2, '{"allows_performance_bonus": {"new": false, "old": true}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (5, NULL, 1, '2026-08-18 17:03:40.588329+00', 40, 2, '{"note": {"new": "Calculation target only: work lands here after the scheme is applied, so it is never selected directly. Still payable — the rules that remap onto it are what make it appear on the payroll sheet.", "old": "Fixed coefficient: every shift and every trade is worth the same under this scheme."}, "is_allowed": {"new": false, "old": true}, "effective_category_id": {"new": null, "old": 1}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (6, NULL, 1, '2026-08-18 17:03:40.633101+00', 26, 2, '{"valid_from": {"new": "2020-01-01", "old": "2026-08-01"}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (7, NULL, 1, '2026-08-18 17:03:40.633101+00', 40, 2, '{"valid_from": {"new": "2020-01-01", "old": "2026-08-01"}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (8, NULL, 1, '2026-08-18 17:03:40.633101+00', 40, 2, '{"note": {"new": "Fixed coefficient. Selectable like any other work category — the supervisor enters what was worked and the backend maps it.", "old": "Calculation target only: work lands here after the scheme is applied, so it is never selected directly. Still payable — the rules that remap onto it are what make it appear on the payroll sheet."}, "is_allowed": {"new": true, "old": false}, "effective_category_id": {"new": 1, "old": null}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (9, NULL, 1, '2026-08-18 17:03:40.662444+00', 40, 2, '{"note": {"new": "Calculation target. Work becomes this AFTER the mapping, so it is fully defined here but never offered for selection.", "old": "Fixed coefficient. Selectable like any other work category — the supervisor enters what was worked and the backend maps it."}, "is_selectable": {"new": false, "old": true}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (10, NULL, 3, '2026-08-18 17:03:40.951405+00', 38, 1, '{"id": 3, "code": "COMMERCIAL", "name": "Komercijala", "note": "Work categories behave as under STANDARD. No hourly bonus: allows_performance_bonus = false zeroes the category bonus, and the MONTHLY_BONUS rule keeps the line visible at zero.", "is_active": true, "archived_at": null, "allows_performance_bonus": false, "allow_unmapped_categories": true}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (11, NULL, 1, '2026-08-18 17:03:41.090818+00', 49, 1, '{"id": 1, "code": "MANUAL_CORRECTION", "name": "Ručna korekcija vremena", "is_active": true, "sort_order": 10, "archived_at": null, "description": "A correction an administrator enters by hand. This is what payroll_run_items.manual_adjusted_minutes has always meant; the row exists so that meaning has a name.", "impact_code": "PAYABLE_MINUTES", "visible_in_ui": true, "allow_negative": true, "allow_positive": true, "require_reason": true, "visible_in_pdf": true, "calculation_key": "MANUAL"}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (12, NULL, 1, '2026-08-18 17:03:41.092561+00', 49, 2, '{"require_reason": {"new": false, "old": true}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (13, NULL, 1, '2026-08-18 17:03:41.092561+00', 49, 2, '{"require_reason": {"new": true, "old": false}}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (14, NULL, 2, '2026-08-18 17:03:41.114204+00', 40, 1, '{"id": 2, "note": "S je cilj remapiranja za FOREIGN_FIXED_COEFFICIENT. Radnici na ovom nacinu obracuna evidentiraju rad na J i D, pa je ovde prazan red na listicu.", "is_active": true, "is_allowed": false, "valid_from": "2020-01-01", "archived_at": null, "valid_until": null, "is_selectable": false, "source_category_id": 1, "coefficient_override": null, "effective_category_id": null, "compensation_scheme_id": 1}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (15, NULL, 3, '2026-08-18 17:03:41.114204+00', 40, 1, '{"id": 3, "note": "S je cilj remapiranja za FOREIGN_FIXED_COEFFICIENT. Radnici na ovom nacinu obracuna evidentiraju rad na J i D, pa je ovde prazan red na listicu.", "is_active": true, "is_allowed": false, "valid_from": "2020-01-01", "archived_at": null, "valid_until": null, "is_selectable": false, "source_category_id": 1, "coefficient_override": null, "effective_category_id": null, "compensation_scheme_id": 3}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (16, NULL, 5, '2026-08-18 17:03:41.454502+00', 44, 1, '{"id": 5, "code": "TRANSPORT_PER_DAY", "name": "Prevoz po danu", "is_active": true, "is_system": true, "unit_code": null, "value_type": "BOOLEAN", "archived_at": null, "description": "TRUE means the employee is paid transport for each worked day, at the company rate app_settings.transport_allowance_per_day in force on the last day of the payroll month. Having this value in force is what puts an employee on the per-day mode, exactly as TRANSPORT_FIXED_MONTHLY does for the fixed mode. An employee with neither is paid no transport.", "payroll_adjustment_category_id": null}');
INSERT INTO public.audit_logs OVERRIDING SYSTEM VALUE VALUES (17, NULL, 1, '2026-08-18 17:03:41.769332+00', 5, 1, '{"id": 1, "name": "Komercijala", "is_active": true, "archived_at": null, "description": "Prodaja i komercijalni poslovi. Nije proizvodni sektor — radnici ovde nemaju normu."}');
...
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (1, 'absence_records');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (2, 'app_settings');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (3, 'bonus_categories');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (4, 'bonus_eligibility_rules');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (5, 'departments');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (6, 'employees');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (7, 'employees_bonus_history');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (8, 'livac_categories');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (9, 'manufacturing_product_times');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (10, 'manufacturing_time_operations');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (13, 'operations');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (14, 'operations_history');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (15, 'plastic_categories');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (16, 'production_order_line_items');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (17, 'production_orders');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (18, 'products');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (20, 'roles');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (21, 'sample_order_line_items');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (22, 'sample_orders');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (23, 'scraps');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (24, 'shifts');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (25, 'users');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (26, 'work_code_categories');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (27, 'work_logs');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (28, 'work_shifts');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (29, 'work_code_category_mappings');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (11, 'manufacturing_time_requests_legacy');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (19, 'requests_legacy');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (30, 'product_manufacturing_times');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (31, 'manufacturing_time_requests');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (12, 'notifications_legacy');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (32, 'user_registration_requests');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (33, 'mailing_lists');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (34, 'mailing_list_members');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (35, 'production_order_mailing_lists');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (36, 'production_order_recipients');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (37, 'notification_events');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (38, 'compensation_schemes');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (39, 'employee_compensation_scheme_history');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (40, 'work_code_category_scheme_rules');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (41, 'work_code_category_translations');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (42, 'payroll_adjustment_category_translations');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (43, 'payroll_adjustment_category_scheme_rules');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (44, 'employee_payroll_value_definitions');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (45, 'employee_payroll_value_history');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (46, 'payroll_adjustments');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (47, 'payroll_adjustment_categories');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (48, 'payroll_run_items');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (49, 'payroll_time_adjustment_categories');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (50, 'payroll_time_adjustments');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (51, 'work_code_category_mapping_types');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (52, 'employee_employment_periods');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (53, 'department_head_periods');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (54, 'employee_work_category_periods');
INSERT INTO public.audit_tables OVERRIDING SYSTEM VALUE VALUES (55, 'operation_norm_versions');


--
-- Data for Name: bonus_categories; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: bonus_eligibility_rules; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: bonus_min_hours_rules; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: compensation_schemes; Type: TABLE DATA; Schema: public; Owner: -
--

INSERT INTO public.compensation_schemes OVERRIDING SYSTEM VALUE VALUES (1, 'STANDARD', 'Standardni obračun', true, 'Default policy. Categories behave exactly as they did before compensation schemes existed: every active category is selectable and the coefficient comes from work_code_categories.norm_multiplier.', true, '2026-08-18 17:03:40.314867+00', NULL, NULL, true);
INSERT INTO public.compensation_schemes OVERRIDING SYSTEM VALUE VALUES (2, 'FOREIGN_FIXED_COEFFICIENT', 'Fiksni koeficijent', false, 'Restricted policy: only categories with an explicit work_code_category_scheme_rules row may be selected, and the shift categories resolve to a single effective category with coefficient 1.', true, '2026-08-18 17:03:40.315676+00', '2026-08-18 17:03:40.563776+00', NULL, false);
INSERT INTO public.compensation_schemes OVERRIDING SYSTEM VALUE VALUES (3, 'COMMERCIAL', 'Komercijala', true, 'Work categories behave as under STANDARD. No hourly bonus: allows_performance_bonus = false zeroes the category bonus, and the MONTHLY_BONUS rule keeps the line visible at zero.', true, '2026-08-18 17:03:40.951405+00', NULL, NULL, false);


--
-- Data for Name: daily_report_categories; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: daily_report_recalc_queue; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: department_head_periods; Type: TABLE DATA; Schema: public; Owner: -
--
...
INSERT INTO public.departments OVERRIDING SYSTEM VALUE VALUES (1, 'Komercijala', 'Prodaja i komercijalni poslovi. Nije proizvodni sektor — radnici ovde nemaju normu.', true, '2026-08-18 17:03:41.769332+00', NULL, NULL);


--
-- Data for Name: employee_compensation_scheme_history; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: employee_employment_periods; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: employee_payroll_run_item_updates; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: employee_payroll_value_definitions; Type: TABLE DATA; Schema: public; Owner: -
--

INSERT INTO public.employee_payroll_value_definitions OVERRIDING SYSTEM VALUE VALUES (1, 'TRANSPORT_FIXED_MONTHLY', 'Fiksna mesečna nadoknada za prevoz', 'A fixed monthly amount, paid whole whatever the employee worked. Having this value is what puts an employee on the fixed mode; everyone else is paid per worked day from app_settings.transport_allowance_per_day.', 'NUMERIC', 'RSD', NULL, true, true, '2026-08-18 17:03:40.697429+00', NULL, NULL);
INSERT INTO public.employee_payroll_value_definitions OVERRIDING SYSTEM VALUE VALUES (2, 'FIXED_LD_AMOUNT', 'Fiksni lični dohodak', 'A fixed monthly amount that replaces the hourly calculation.', 'NUMERIC', 'RSD', NULL, true, true, '2026-08-18 17:03:40.697429+00', NULL, NULL);
INSERT INTO public.employee_payroll_value_definitions OVERRIDING SYSTEM VALUE VALUES (3, 'TELEPHONE_AMOUNT', 'Iznos za telefon', 'The employee''s standing telephone deduction.', 'NUMERIC', 'RSD', NULL, true, true, '2026-08-18 17:03:40.697429+00', NULL, NULL);
INSERT INTO public.employee_payroll_value_definitions OVERRIDING SYSTEM VALUE VALUES (4, 'HOURLY_RATE', 'Satnica', 'Prices payroll_run_item_categories. A calculation input, not a payslip line, so it has no adjustment category.', 'NUMERIC', 'RSD', NULL, true, true, '2026-08-18 17:03:40.697429+00', NULL, NULL);
INSERT INTO public.employee_payroll_value_definitions OVERRIDING SYSTEM VALUE VALUES (5, 'TRANSPORT_PER_DAY', 'Prevoz po danu', 'TRUE means the employee is paid transport for each worked day, at the company rate app_settings.transport_allowance_per_day in force on the last day of the payroll month. Having this value in force is what puts an employee on the per-day mode, exactly as TRANSPORT_FIXED_MONTHLY does for the fixed mode. An employee with neither is paid no transport.', 'BOOLEAN', NULL, NULL, true, true, '2026-08-18 17:03:41.454502+00', NULL, NULL);


--
-- Data for Name: employee_payroll_value_history; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: employee_record_updates; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: employee_records; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: employee_work_category_periods; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: employees_bonus_history; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: livac_categories; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: mailing_list_access; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: mailing_list_members; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: mailing_lists; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: manufacturing_product_times; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: manufacturing_time_operations; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: manufacturing_time_requests; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: monthly_report_categories; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: monthly_report_recalc_queue; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: operation_norm_versions; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: payroll_adjustment_categories; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: payroll_adjustment_category_scheme_rules; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: payroll_adjustment_category_translations; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: payroll_field_access; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: payroll_run_item_categories; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: payroll_run_item_handovers; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: payroll_run_items; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: payroll_time_adjustment_categories; Type: TABLE DATA; Schema: public; Owner: -
--

INSERT INTO public.payroll_time_adjustment_categories OVERRIDING SYSTEM VALUE VALUES (1, 'MANUAL_CORRECTION', 'Ručna korekcija vremena', 'A correction an administrator enters by hand. This is what payroll_run_items.manual_adjusted_minutes has always meant; the row exists so that meaning has a name.', 'PAYABLE_MINUTES', 'MANUAL', true, true, true, true, true, 10, true, '2026-08-18 17:03:41.090818+00', '2026-08-18 17:03:41.092561+00', NULL);


--
-- Data for Name: payroll_time_adjustments; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: plastic_categories; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: product_manufacturing_time_operations; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: product_manufacturing_times; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: production_order_deadlines; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: production_order_line_item_notes; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: production_order_line_item_quantities; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: production_order_line_items; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: production_order_mailing_lists; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: production_order_recipients; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: sample_order_line_item_notes; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: sample_order_line_item_quantities; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: sample_order_line_items; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: sample_orders; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: scraps; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: user_registration_requests; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: user_saved_views; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: user_table_preferences; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Data for Name: work_calendar_days; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: work_code_categories; Type: TABLE DATA; Schema: public; Owner: -
--

INSERT INTO public.work_code_categories OVERRIDING SYSTEM VALUE VALUES (1, 'S', 'I, II i III smena', 'WORK', 1, true, '2020-01-01', NULL, '2026-08-18 17:03:40.374467+00', '2026-08-18 17:03:40.633101+00', NULL, true, true, true, 'Common effective category for the FOREIGN_FIXED_COEFFICIENT scheme. Never selected directly on a work log: it is resolved from a source category (J, D, ...) by work_code_category_scheme_rules. The source category is always preserved.', NULL, false, 1, true, true, false, true);


--
-- Data for Name: work_code_category_mapping_types; Type: TABLE DATA; Schema: public; Owner: -
--

INSERT INTO public.work_code_category_mapping_types OVERRIDING SYSTEM VALUE VALUES (1, 'MULTIPLE_MACHINES_BONUS', 'Bonus za više mašina', true, true, 'Applies from the first day; probation does not affect it.', '2026-08-18 17:03:41.57783+00', NULL);
INSERT INTO public.work_code_category_mapping_types OVERRIDING SYSTEM VALUE VALUES (2, 'WEEKEND_BONUS', 'Bonus za rad vikendom', false, true, 'Not paid while the employee is on probation — owner rule, in force always.', '2026-08-18 17:03:41.57783+00', NULL);
INSERT INTO public.work_code_category_mapping_types OVERRIDING SYSTEM VALUE VALUES (3, 'NIGHT_SHIFT_BONUS', 'Bonus za noćnu smenu', true, true, 'Applies from the first day; probation does not affect it.', '2026-08-18 17:03:41.57783+00', NULL);


--
-- Data for Name: work_code_category_mappings; Type: TABLE DATA; Schema: public; Owner: -
--



--
-- Data for Name: work_code_category_scheme_rules; Type: TABLE DATA; Schema: public; Owner: -
--

INSERT INTO public.work_code_category_scheme_rules OVERRIDING SYSTEM VALUE VALUES (1, 2, 1, 1, true, 1.00, '2020-01-01', NULL, 'Calculation target. Work becomes this AFTER the mapping, so it is fully defined here but never offered for selection.', true, '2026-08-18 17:03:40.528183+00', '2026-08-18 17:03:40.662444+00', NULL, false);
INSERT INTO public.work_code_category_scheme_rules OVERRIDING SYSTEM VALUE VALUES (2, 1, 1, NULL, false, NULL, '2020-01-01', NULL, 'S je cilj remapiranja za FOREIGN_FIXED_COEFFICIENT. Radnici na ovom nacinu obracuna evidentiraju rad na J i D, pa je ovde prazan red na listicu.', true, '2026-08-18 17:03:41.114204+00', NULL, NULL, false);
INSERT INTO public.work_code_category_scheme_rules OVERRIDING SYSTEM VALUE VALUES (3, 3, 1, NULL, false, NULL, '2020-01-01', NULL, 'S je cilj remapiranja za FOREIGN_FIXED_COEFFICIENT. Radnici na ovom nacinu obracuna evidentiraju rad na J i D, pa je ovde prazan red na listicu.', true, '2026-08-18 17:03:41.114204+00', NULL, NULL, false);


--
-- Data for Name: work_code_category_translations; Type: TABLE DATA; Schema: public; Owner: -
--

INSERT INTO public.work_code_category_translations OVERRIDING SYSTEM VALUE VALUES (1, 1, 'en', '1st, 2nd and 3rd shift', '2026-08-18 17:03:40.404905+00', NULL);


--
-- Data for Name: work_log_facts; Type: TABLE DATA; Schema: public; Owner: -
--
...
--
-- Name: absence_compensations_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.absence_compensations_id_seq', 1, false);


--
-- Name: absence_records_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.absence_records_id_seq', 1, false);
...
SELECT pg_catalog.setval('public.audit_tables_id_seq', 55, true);


--
-- Name: bonus_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.bonus_categories_id_seq', 1, false);


--
-- Name: bonus_eligibility_rules_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.bonus_eligibility_rules_id_seq', 1, false);


--
-- Name: bonus_min_hours_rules_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.bonus_min_hours_rules_id_seq', 1, false);


--
-- Name: compensation_schemes_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.compensation_schemes_id_seq', 3, true);


--
-- Name: daily_report_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.daily_report_categories_id_seq', 1, false);


--
-- Name: daily_report_recalc_queue_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.daily_report_recalc_queue_id_seq', 1, false);
...
--
-- Name: department_head_periods_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.department_head_periods_id_seq', 1, false);
...
--
-- Name: employee_compensation_scheme_history_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_compensation_scheme_history_id_seq', 1, false);


--
-- Name: employee_employment_periods_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_employment_periods_id_seq', 1, false);


--
-- Name: employee_payroll_run_item_updates_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_payroll_run_item_updates_id_seq', 1, false);


--
-- Name: employee_payroll_value_definitions_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_payroll_value_definitions_id_seq', 5, true);


--
-- Name: employee_payroll_value_history_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_payroll_value_history_id_seq', 1, false);


--
-- Name: employee_record_updates_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_record_updates_id_seq', 1, false);


--
-- Name: employee_records_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_records_id_seq', 1, false);


--
-- Name: employee_work_category_periods_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employee_work_category_periods_id_seq', 1, false);


--
-- Name: employees_bonus_history_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.employees_bonus_history_id_seq', 1, false);
...
--
-- Name: livac_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.livac_categories_id_seq', 1, false);


--
-- Name: mailing_list_members_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.mailing_list_members_id_seq', 1, false);


--
-- Name: mailing_lists_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.mailing_lists_id_seq', 1, false);


--
-- Name: manufacturing_product_times_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.manufacturing_product_times_id_seq', 1, false);


--
-- Name: manufacturing_time_operations_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.manufacturing_time_operations_id_seq', 1, false);


--
-- Name: manufacturing_time_requests_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.manufacturing_time_requests_id_seq', 1, false);


--
-- Name: monthly_report_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.monthly_report_categories_id_seq', 1, false);


--
-- Name: monthly_report_recalc_queue_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.monthly_report_recalc_queue_id_seq', 1, false);
...
--
-- Name: operation_norm_versions_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.operation_norm_versions_id_seq', 1, false);


--
-- Name: operations_history_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.operations_history_id_seq', 1, false);
...
--
-- Name: payroll_adjustment_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_adjustment_categories_id_seq', 1, false);


--
-- Name: payroll_adjustment_category_scheme_rules_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_adjustment_category_scheme_rules_id_seq', 1, false);


--
-- Name: payroll_adjustment_category_translations_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_adjustment_category_translations_id_seq', 1, false);
...
--
-- Name: payroll_field_access_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_field_access_id_seq', 1, false);


--
-- Name: payroll_run_item_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_run_item_categories_id_seq', 1, false);


--
-- Name: payroll_run_item_handovers_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_run_item_handovers_id_seq', 1, false);


--
-- Name: payroll_run_items_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_run_items_id_seq', 1, false);
...
--
-- Name: payroll_time_adjustment_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_time_adjustment_categories_id_seq', 1, true);


--
-- Name: payroll_time_adjustments_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.payroll_time_adjustments_id_seq', 1, false);


--
-- Name: plastic_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.plastic_categories_id_seq', 1, false);


--
-- Name: product_manufacturing_time_operations_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.product_manufacturing_time_operations_id_seq', 1, false);


--
-- Name: product_manufacturing_times_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.product_manufacturing_times_id_seq', 1, false);


--
-- Name: production_order_deadlines_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.production_order_deadlines_id_seq', 1, false);


--
-- Name: production_order_line_item_notes_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.production_order_line_item_notes_id_seq', 1, false);


--
-- Name: production_order_line_item_quantities_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.production_order_line_item_quantities_id_seq', 1, false);


--
-- Name: production_order_line_items_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.production_order_line_items_id_seq', 1, false);


--
-- Name: production_order_mailing_lists_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.production_order_mailing_lists_id_seq', 1, false);


--
-- Name: production_order_recipients_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.production_order_recipients_id_seq', 1, false);
...
--
-- Name: sample_order_line_item_notes_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.sample_order_line_item_notes_id_seq', 1, false);


--
-- Name: sample_order_line_item_quantities_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.sample_order_line_item_quantities_id_seq', 1, false);


--
-- Name: sample_order_line_items_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.sample_order_line_items_id_seq', 1, false);


--
-- Name: sample_orders_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.sample_orders_id_seq', 1, false);


--
-- Name: scraps_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.scraps_id_seq', 1, false);
...
--
-- Name: user_registration_requests_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.user_registration_requests_id_seq', 1, false);


--
-- Name: user_saved_views_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.user_saved_views_id_seq', 1, false);
...
--
-- Name: user_table_preferences_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.user_table_preferences_id_seq', 1, false);
...
--
-- Name: work_calendar_days_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.work_calendar_days_id_seq', 1, false);


--
-- Name: work_code_categories_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.work_code_categories_id_seq', 1, true);


--
-- Name: work_code_category_mapping_types_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.work_code_category_mapping_types_id_seq', 3, true);


--
-- Name: work_code_category_mappings_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.work_code_category_mappings_id_seq', 1, false);


--
-- Name: work_code_category_scheme_rules_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.work_code_category_scheme_rules_id_seq', 3, true);


--
-- Name: work_code_category_translations_id_seq; Type: SEQUENCE SET; Schema: public; Owner: -
--

SELECT pg_catalog.setval('public.work_code_category_translations_id_seq', 1, true);
...
--
-- Name: compensation_schemes compensation_schemes_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.compensation_schemes
    ADD CONSTRAINT compensation_schemes_pkey PRIMARY KEY (id);
...
--
-- Name: department_head_periods department_head_periods_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.department_head_periods
    ADD CONSTRAINT department_head_periods_pkey PRIMARY KEY (id);
...
--
-- Name: employee_compensation_scheme_history employee_compensation_scheme_history_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_compensation_scheme_history
    ADD CONSTRAINT employee_compensation_scheme_history_pkey PRIMARY KEY (id);


--
-- Name: employee_employment_periods employee_employment_periods_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_employment_periods
    ADD CONSTRAINT employee_employment_periods_pkey PRIMARY KEY (id);
...
--
-- Name: employee_payroll_value_definitions employee_payroll_value_definitions_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_payroll_value_definitions
    ADD CONSTRAINT employee_payroll_value_definitions_pkey PRIMARY KEY (id);


--
-- Name: employee_payroll_value_history employee_payroll_value_history_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_payroll_value_history
    ADD CONSTRAINT employee_payroll_value_history_pkey PRIMARY KEY (id);
...
--
-- Name: employee_work_category_periods employee_work_category_periods_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_work_category_periods
    ADD CONSTRAINT employee_work_category_periods_pkey PRIMARY KEY (id);
...
--
-- Name: department_head_periods ex_dhp_no_overlap; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.department_head_periods
    ADD CONSTRAINT ex_dhp_no_overlap EXCLUDE USING gist (department_id WITH =, COALESCE(shift_id, ('-1'::integer)::bigint) WITH =, daterange(valid_from,
CASE
    WHEN (valid_to IS NULL) THEN NULL::date
    ELSE (valid_to + 1)
END) WITH &&) WHERE ((archived_at IS NULL));


--
-- Name: employee_compensation_scheme_history ex_ecsh_no_overlap; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_compensation_scheme_history
    ADD CONSTRAINT ex_ecsh_no_overlap EXCLUDE USING gist (employee_id WITH =, daterange(valid_from,
CASE
    WHEN (valid_until IS NULL) THEN NULL::date
    ELSE (valid_until + 1)
END) WITH &&) WHERE ((archived_at IS NULL));


--
-- Name: employee_employment_periods ex_eep_no_overlap; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_employment_periods
    ADD CONSTRAINT ex_eep_no_overlap EXCLUDE USING gist (employee_id WITH =, daterange(started_on,
CASE
    WHEN (ended_on IS NULL) THEN NULL::date
    ELSE (ended_on + 1)
END) WITH &&) WHERE ((archived_at IS NULL));
...
--
-- Name: employee_payroll_value_history ex_epvh_no_overlap; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_payroll_value_history
    ADD CONSTRAINT ex_epvh_no_overlap EXCLUDE USING gist (employee_id WITH =, value_definition_id WITH =, daterange(valid_from,
CASE
    WHEN (valid_until IS NULL) THEN NULL::date
    ELSE (valid_until + 1)
END) WITH &&) WHERE ((archived_at IS NULL));
...
--
-- Name: employee_work_category_periods ex_ewcp_no_overlap; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_work_category_periods
    ADD CONSTRAINT ex_ewcp_no_overlap EXCLUDE USING gist (employee_id WITH =, daterange(valid_from,
CASE
    WHEN (valid_to IS NULL) THEN NULL::date
    ELSE (valid_to + 1)
END) WITH &&) WHERE ((archived_at IS NULL));
...
--
-- Name: payroll_adjustment_category_scheme_rules ex_pacsr_no_overlap; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_adjustment_category_scheme_rules
    ADD CONSTRAINT ex_pacsr_no_overlap EXCLUDE USING gist (compensation_scheme_id WITH =, payroll_adjustment_category_id WITH =, daterange(valid_from,
CASE
    WHEN (valid_until IS NULL) THEN NULL::date
    ELSE (valid_until + 1)
END) WITH &&) WHERE (((archived_at IS NULL) AND (is_active = true)));
...
--
-- Name: work_code_category_scheme_rules ex_wccsr_no_overlap; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_scheme_rules
    ADD CONSTRAINT ex_wccsr_no_overlap EXCLUDE USING gist (compensation_scheme_id WITH =, source_category_id WITH =, daterange(valid_from,
CASE
    WHEN (valid_until IS NULL) THEN NULL::date
    ELSE (valid_until + 1)
END) WITH &&) WHERE (((archived_at IS NULL) AND (is_active = true)));
...
ALTER TABLE ONLY public.work_shifts
    ADD CONSTRAINT ex_work_shifts_no_overlap EXCLUDE USING gist (employee_id WITH =, tstzrange(start_at, end_at) WITH &&) WHERE ((archived_at IS NULL));
...
--
-- Name: operation_norm_versions operation_norm_versions_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.operation_norm_versions
    ADD CONSTRAINT operation_norm_versions_pkey PRIMARY KEY (id);
...
--
-- Name: payroll_adjustment_category_scheme_rules payroll_adjustment_category_scheme_rules_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_adjustment_category_scheme_rules
    ADD CONSTRAINT payroll_adjustment_category_scheme_rules_pkey PRIMARY KEY (id);


--
-- Name: payroll_adjustment_category_translations payroll_adjustment_category_translations_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_adjustment_category_translations
    ADD CONSTRAINT payroll_adjustment_category_translations_pkey PRIMARY KEY (id);
...
--
-- Name: payroll_field_access payroll_field_access_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_field_access
    ADD CONSTRAINT payroll_field_access_pkey PRIMARY KEY (id);
...
--
-- Name: payroll_run_item_handovers payroll_run_item_handovers_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_run_item_handovers
    ADD CONSTRAINT payroll_run_item_handovers_pkey PRIMARY KEY (id);
...
--
-- Name: payroll_time_adjustment_categories payroll_time_adjustment_categories_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_time_adjustment_categories
    ADD CONSTRAINT payroll_time_adjustment_categories_pkey PRIMARY KEY (id);


--
-- Name: payroll_time_adjustments payroll_time_adjustments_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_time_adjustments
    ADD CONSTRAINT payroll_time_adjustments_pkey PRIMARY KEY (id);
...
--
-- Name: compensation_schemes uq_compensation_schemes_code; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.compensation_schemes
    ADD CONSTRAINT uq_compensation_schemes_code UNIQUE (code);
...
--
-- Name: employee_payroll_value_definitions uq_epvd_id_value_type; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: payroll_adjustments uq_payroll_adjustment_item_category; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_adjustments
    ADD CONSTRAINT uq_payroll_adjustment_item_category UNIQUE (payroll_run_item_id, payroll_adjustment_category_id);
...
--
-- Name: payroll_field_access uq_pfa_field_role; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_field_access
    ADD CONSTRAINT uq_pfa_field_role UNIQUE (field_code, role_name);
...
--
-- Name: work_code_category_mapping_types uq_wcmt_code; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_mapping_types
    ADD CONSTRAINT uq_wcmt_code UNIQUE (code);
...
--
-- Name: work_code_category_mapping_types work_code_category_mapping_types_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_mapping_types
    ADD CONSTRAINT work_code_category_mapping_types_pkey PRIMARY KEY (id);
...
--
-- Name: work_code_category_scheme_rules work_code_category_scheme_rules_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_scheme_rules
    ADD CONSTRAINT work_code_category_scheme_rules_pkey PRIMARY KEY (id);


--
-- Name: work_code_category_translations work_code_category_translations_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_translations
    ADD CONSTRAINT work_code_category_translations_pkey PRIMARY KEY (id);
...
--
-- Name: idx_daily_reports_probation; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_daily_reports_probation ON public.daily_reports USING btree (employee_id, work_date) WHERE was_probation;


--
-- Name: idx_dhp_department_valid_from; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_dhp_department_valid_from ON public.department_head_periods USING btree (department_id, valid_from DESC) WHERE (archived_at IS NULL);


--
-- Name: idx_dhp_employee_valid_from; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_dhp_employee_valid_from ON public.department_head_periods USING btree (employee_id, valid_from DESC) WHERE (archived_at IS NULL);


--
-- Name: idx_ecsh_employee_period; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_ecsh_employee_period ON public.employee_compensation_scheme_history USING btree (employee_id, valid_from, valid_until) WHERE (archived_at IS NULL);


--
-- Name: idx_ecsh_scheme; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_ecsh_scheme ON public.employee_compensation_scheme_history USING btree (compensation_scheme_id);


--
-- Name: idx_eep_employee_started; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_eep_employee_started ON public.employee_employment_periods USING btree (employee_id, started_on DESC) WHERE (archived_at IS NULL);
...
--
-- Name: idx_epvh_definition; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_epvh_definition ON public.employee_payroll_value_history USING btree (value_definition_id);


--
-- Name: idx_epvh_employee_period; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_epvh_employee_period ON public.employee_payroll_value_history USING btree (employee_id, valid_from DESC) WHERE (archived_at IS NULL);


--
-- Name: idx_ewcp_employee_valid_from; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_ewcp_employee_valid_from ON public.employee_work_category_periods USING btree (employee_id, valid_from DESC) WHERE (archived_at IS NULL);
...
--
-- Name: idx_operation_norm_versions_operation; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_operation_norm_versions_operation ON public.operation_norm_versions USING btree (operation_id, created_at DESC);
...
--
-- Name: idx_pacsr_lookup; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pacsr_lookup ON public.payroll_adjustment_category_scheme_rules USING btree (compensation_scheme_id, valid_from, valid_until) WHERE ((archived_at IS NULL) AND (is_active = true));


--
-- Name: idx_pact_locale; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pact_locale ON public.payroll_adjustment_category_translations USING btree (lower((locale)::text));
...
--
-- Name: idx_pfa_role; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pfa_role ON public.payroll_field_access USING btree (role_name);
...
--
-- Name: idx_prih_item_occurred; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_prih_item_occurred ON public.payroll_run_item_handovers USING btree (payroll_run_item_id, occurred_at DESC);
...
--
-- Name: idx_pta_category; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pta_category ON public.payroll_time_adjustments USING btree (payroll_time_adjustment_category_id);


--
-- Name: idx_pta_item; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pta_item ON public.payroll_time_adjustments USING btree (payroll_run_item_id) WHERE (archived_at IS NULL);
...
--
-- Name: idx_wccsr_lookup; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wccsr_lookup ON public.work_code_category_scheme_rules USING btree (compensation_scheme_id, source_category_id, valid_from, valid_until) WHERE ((archived_at IS NULL) AND (is_active = true));


--
-- Name: idx_wccsr_scheme_active; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wccsr_scheme_active ON public.work_code_category_scheme_rules USING btree (compensation_scheme_id, valid_from, valid_until) WHERE ((archived_at IS NULL) AND (is_active = true));


--
-- Name: idx_wcct_locale; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wcct_locale ON public.work_code_category_translations USING btree (lower((locale)::text));
...
--
-- Name: idx_work_logs_scheme_effective_category; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_work_logs_scheme_effective_category ON public.work_logs USING btree (scheme_effective_work_code_category_id) WHERE (scheme_effective_work_code_category_id IS NOT NULL);
...
--
-- Name: idx_work_shifts_archived; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_work_shifts_archived ON public.work_shifts USING btree (archived_at) WHERE (archived_at IS NOT NULL);
...
--
-- Name: uq_epvd_code; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_epvd_code ON public.employee_payroll_value_definitions USING btree (code);
...
--
-- Name: uq_pact_category_locale; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_pact_category_locale ON public.payroll_adjustment_category_translations USING btree (payroll_adjustment_category_id, lower((locale)::text));
...
--
-- Name: uq_ptac_code; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_ptac_code ON public.payroll_time_adjustment_categories USING btree (code);
...
--
-- Name: uq_wcct_category_locale; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_wcct_category_locale ON public.work_code_category_translations USING btree (work_code_category_id, lower((locale)::text));
...
--
-- Name: uq_work_shifts_employee_shift_work_date; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uq_work_shifts_employee_shift_work_date ON public.work_shifts USING btree (employee_id, shift_id, work_date) WHERE (archived_at IS NULL);


--
-- Name: ux_employees_jmbg; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_employees_jmbg ON public.employees USING btree (jmbg) WHERE (jmbg IS NOT NULL);
...
--
-- Name: payroll_time_adjustments trg_01_pta_require_reason; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_01_pta_require_reason BEFORE INSERT OR UPDATE ON public.payroll_time_adjustments FOR EACH ROW EXECUTE FUNCTION public.payroll_time_adjustment_require_reason();
...
--
-- Name: compensation_schemes trg_02_compensation_schemes_archived_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_02_compensation_schemes_archived_at BEFORE UPDATE ON public.compensation_schemes FOR EACH ROW EXECUTE FUNCTION public.set_archived_at_on_deactivate();
...
--
-- Name: payroll_adjustment_category_scheme_rules trg_02_pacsr_archived_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_02_pacsr_archived_at BEFORE UPDATE ON public.payroll_adjustment_category_scheme_rules FOR EACH ROW EXECUTE FUNCTION public.set_archived_at_on_deactivate();
...
--
-- Name: work_code_category_scheme_rules trg_02_wccsr_archived_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_02_wccsr_archived_at BEFORE UPDATE ON public.work_code_category_scheme_rules FOR EACH ROW EXECUTE FUNCTION public.set_archived_at_on_deactivate();
...
--
-- Name: compensation_schemes trg_03_compensation_schemes_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_compensation_schemes_updated_at BEFORE UPDATE ON public.compensation_schemes FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: department_head_periods trg_03_dhp_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_dhp_updated_at BEFORE UPDATE ON public.department_head_periods FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: employee_compensation_scheme_history trg_03_ecsh_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_ecsh_updated_at BEFORE UPDATE ON public.employee_compensation_scheme_history FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: employee_employment_periods trg_03_eep_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_eep_updated_at BEFORE UPDATE ON public.employee_employment_periods FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: employee_payroll_value_definitions trg_03_epvd_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_epvd_updated_at BEFORE UPDATE ON public.employee_payroll_value_definitions FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: employee_payroll_value_history trg_03_epvh_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_epvh_updated_at BEFORE UPDATE ON public.employee_payroll_value_history FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: employee_work_category_periods trg_03_ewcp_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_ewcp_updated_at BEFORE UPDATE ON public.employee_work_category_periods FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: payroll_adjustment_category_scheme_rules trg_03_pacsr_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_pacsr_updated_at BEFORE UPDATE ON public.payroll_adjustment_category_scheme_rules FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: payroll_adjustment_category_translations trg_03_pact_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_pact_updated_at BEFORE UPDATE ON public.payroll_adjustment_category_translations FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: payroll_time_adjustment_categories trg_03_payroll_time_adjustment_categories_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_payroll_time_adjustment_categories_updated_at BEFORE UPDATE ON public.payroll_time_adjustment_categories FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: payroll_time_adjustments trg_03_payroll_time_adjustments_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_payroll_time_adjustments_updated_at BEFORE UPDATE ON public.payroll_time_adjustments FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: work_code_category_scheme_rules trg_03_wccsr_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_wccsr_updated_at BEFORE UPDATE ON public.work_code_category_scheme_rules FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: work_code_category_translations trg_03_wcct_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_wcct_updated_at BEFORE UPDATE ON public.work_code_category_translations FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();


--
-- Name: work_code_category_mapping_types trg_03_wcmt_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_03_wcmt_updated_at BEFORE UPDATE ON public.work_code_category_mapping_types FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: employee_work_category_periods trg_04_ewcp_sync_employee; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_04_ewcp_sync_employee AFTER INSERT OR DELETE OR UPDATE ON public.employee_work_category_periods FOR EACH ROW EXECUTE FUNCTION public.work_category_period_sync_employee_fn();
...
--
-- Name: compensation_schemes trg_audit_logs_compensation_schemes; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_compensation_schemes AFTER INSERT OR DELETE OR UPDATE ON public.compensation_schemes FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: department_head_periods trg_audit_logs_department_head_periods; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_department_head_periods AFTER INSERT OR DELETE OR UPDATE ON public.department_head_periods FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();
...
--
-- Name: employee_compensation_scheme_history trg_audit_logs_employee_compensation_scheme_history; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_employee_compensation_scheme_history AFTER INSERT OR DELETE OR UPDATE ON public.employee_compensation_scheme_history FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: employee_employment_periods trg_audit_logs_employee_employment_periods; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_employee_employment_periods AFTER INSERT OR DELETE OR UPDATE ON public.employee_employment_periods FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: employee_payroll_value_definitions trg_audit_logs_employee_payroll_value_definitions; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_employee_payroll_value_definitions AFTER INSERT OR DELETE OR UPDATE ON public.employee_payroll_value_definitions FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: employee_payroll_value_history trg_audit_logs_employee_payroll_value_history; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_employee_payroll_value_history AFTER INSERT OR DELETE OR UPDATE ON public.employee_payroll_value_history FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: employee_work_category_periods trg_audit_logs_employee_work_category_periods; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_employee_work_category_periods AFTER INSERT OR DELETE OR UPDATE ON public.employee_work_category_periods FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();
...
--
-- Name: operation_norm_versions trg_audit_logs_operation_norm_versions; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_operation_norm_versions AFTER INSERT OR DELETE OR UPDATE ON public.operation_norm_versions FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();
...
--
-- Name: payroll_adjustment_categories trg_audit_logs_payroll_adjustment_categories; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_payroll_adjustment_categories AFTER INSERT OR DELETE OR UPDATE ON public.payroll_adjustment_categories FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: payroll_adjustment_category_scheme_rules trg_audit_logs_payroll_adjustment_category_scheme_rules; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_payroll_adjustment_category_scheme_rules AFTER INSERT OR DELETE OR UPDATE ON public.payroll_adjustment_category_scheme_rules FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: payroll_adjustment_category_translations trg_audit_logs_payroll_adjustment_category_translations; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_payroll_adjustment_category_translations AFTER INSERT OR DELETE OR UPDATE ON public.payroll_adjustment_category_translations FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: payroll_adjustments trg_audit_logs_payroll_adjustments; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_payroll_adjustments AFTER INSERT OR DELETE OR UPDATE ON public.payroll_adjustments FOR EACH ROW WHEN ((COALESCE(current_setting('app.records_decision'::text, true), ''::text) = 'true'::text)) EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: payroll_run_items trg_audit_logs_payroll_run_items_human_input; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_payroll_run_items_human_input AFTER UPDATE ON public.payroll_run_items FOR EACH ROW WHEN ((old.hourly_rate_overridden IS DISTINCT FROM new.hourly_rate_overridden)) EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: payroll_time_adjustment_categories trg_audit_logs_payroll_time_adjustment_categories; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_payroll_time_adjustment_categories AFTER INSERT OR DELETE OR UPDATE ON public.payroll_time_adjustment_categories FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: payroll_time_adjustments trg_audit_logs_payroll_time_adjustments; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_payroll_time_adjustments AFTER INSERT OR DELETE OR UPDATE ON public.payroll_time_adjustments FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();
...
--
-- Name: work_code_category_mapping_types trg_audit_logs_work_code_category_mapping_types; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_work_code_category_mapping_types AFTER INSERT OR DELETE OR UPDATE ON public.work_code_category_mapping_types FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();
...
--
-- Name: work_code_category_scheme_rules trg_audit_logs_work_code_category_scheme_rules; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_work_code_category_scheme_rules AFTER INSERT OR DELETE OR UPDATE ON public.work_code_category_scheme_rules FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();


--
-- Name: work_code_category_translations trg_audit_logs_work_code_category_translations; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_logs_work_code_category_translations AFTER INSERT OR DELETE OR UPDATE ON public.work_code_category_translations FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_fn();
...
--
-- Name: compensation_schemes trg_compensation_scheme_activation_insert; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_compensation_scheme_activation_insert BEFORE INSERT ON public.compensation_schemes FOR EACH ROW WHEN (new.is_active) EXECUTE FUNCTION public.compensation_scheme_activation_needs_full_matrix();


--
-- Name: compensation_schemes trg_compensation_scheme_activation_update; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_compensation_scheme_activation_update BEFORE UPDATE ON public.compensation_schemes FOR EACH ROW WHEN ((new.is_active AND (NOT old.is_active))) EXECUTE FUNCTION public.compensation_scheme_activation_needs_full_matrix();


--
-- Name: employee_employment_periods trg_eep_sync_employee; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_eep_sync_employee AFTER INSERT OR DELETE OR UPDATE ON public.employee_employment_periods FOR EACH ROW EXECUTE FUNCTION public.employment_period_sync_employee_fn();


--
-- Name: payroll_run_item_handovers trg_prih_append_only; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_prih_append_only BEFORE DELETE OR UPDATE ON public.payroll_run_item_handovers FOR EACH ROW EXECUTE FUNCTION public.payroll_run_item_handovers_are_append_only();
...
--
-- Name: department_head_periods fk_dhp_department; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.department_head_periods
    ADD CONSTRAINT fk_dhp_department FOREIGN KEY (department_id) REFERENCES public.departments(id);


--
-- Name: department_head_periods fk_dhp_employee; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.department_head_periods
    ADD CONSTRAINT fk_dhp_employee FOREIGN KEY (employee_id) REFERENCES public.employees(id);


--
-- Name: department_head_periods fk_dhp_shift; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.department_head_periods
    ADD CONSTRAINT fk_dhp_shift FOREIGN KEY (shift_id) REFERENCES public.shifts(id);
...
--
-- Name: employee_compensation_scheme_history fk_ecsh_employee; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_compensation_scheme_history
    ADD CONSTRAINT fk_ecsh_employee FOREIGN KEY (employee_id) REFERENCES public.employees(id) ON DELETE CASCADE;


--
-- Name: employee_compensation_scheme_history fk_ecsh_scheme; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_compensation_scheme_history
    ADD CONSTRAINT fk_ecsh_scheme FOREIGN KEY (compensation_scheme_id) REFERENCES public.compensation_schemes(id) ON DELETE RESTRICT;


--
-- Name: employee_employment_periods fk_eep_employee; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_employment_periods
    ADD CONSTRAINT fk_eep_employee FOREIGN KEY (employee_id) REFERENCES public.employees(id);
...
--
-- Name: employee_payroll_value_definitions fk_epvd_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_payroll_value_definitions
    ADD CONSTRAINT fk_epvd_category FOREIGN KEY (payroll_adjustment_category_id) REFERENCES public.payroll_adjustment_categories(id) ON DELETE RESTRICT;


--
-- Name: employee_payroll_value_history fk_epvh_created_by; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_payroll_value_history
    ADD CONSTRAINT fk_epvh_created_by FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: employee_payroll_value_history fk_epvh_definition_type; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_payroll_value_history
    ADD CONSTRAINT fk_epvh_definition_type FOREIGN KEY (value_definition_id, value_type) REFERENCES public.employee_payroll_value_definitions(id, value_type) ON UPDATE CASCADE ON DELETE RESTRICT;


--
-- Name: employee_payroll_value_history fk_epvh_employee; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_payroll_value_history
    ADD CONSTRAINT fk_epvh_employee FOREIGN KEY (employee_id) REFERENCES public.employees(id) ON DELETE CASCADE;


--
-- Name: employee_work_category_periods fk_ewcp_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_work_category_periods
    ADD CONSTRAINT fk_ewcp_category FOREIGN KEY (work_code_category_id) REFERENCES public.work_code_categories(id);


--
-- Name: employee_work_category_periods fk_ewcp_employee; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.employee_work_category_periods
    ADD CONSTRAINT fk_ewcp_employee FOREIGN KEY (employee_id) REFERENCES public.employees(id);
...
--
-- Name: operations fk_operations_archived_by; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.operations
    ADD CONSTRAINT fk_operations_archived_by FOREIGN KEY (archived_by) REFERENCES public.users(id) ON DELETE RESTRICT;
...
--
-- Name: payroll_adjustment_category_scheme_rules fk_pacsr_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_adjustment_category_scheme_rules
    ADD CONSTRAINT fk_pacsr_category FOREIGN KEY (payroll_adjustment_category_id) REFERENCES public.payroll_adjustment_categories(id) ON DELETE RESTRICT;


--
-- Name: payroll_adjustment_category_scheme_rules fk_pacsr_scheme; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_adjustment_category_scheme_rules
    ADD CONSTRAINT fk_pacsr_scheme FOREIGN KEY (compensation_scheme_id) REFERENCES public.compensation_schemes(id) ON DELETE RESTRICT;


--
-- Name: payroll_adjustment_category_translations fk_pact_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_adjustment_category_translations
    ADD CONSTRAINT fk_pact_category FOREIGN KEY (payroll_adjustment_category_id) REFERENCES public.payroll_adjustment_categories(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.payroll_adjustments
    ADD CONSTRAINT fk_payroll_adjustments_pac FOREIGN KEY (payroll_adjustment_category_id) REFERENCES public.payroll_adjustment_categories(id) ON DELETE RESTRICT;
...
--
-- Name: payroll_time_adjustments fk_pta_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_time_adjustments
    ADD CONSTRAINT fk_pta_category FOREIGN KEY (payroll_time_adjustment_category_id) REFERENCES public.payroll_time_adjustment_categories(id) ON DELETE RESTRICT;


--
-- Name: payroll_time_adjustments fk_pta_created_by; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_time_adjustments
    ADD CONSTRAINT fk_pta_created_by FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: payroll_time_adjustments fk_pta_edited_by; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_time_adjustments
    ADD CONSTRAINT fk_pta_edited_by FOREIGN KEY (edited_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: payroll_time_adjustments fk_pta_item; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_time_adjustments
    ADD CONSTRAINT fk_pta_item FOREIGN KEY (payroll_run_item_id) REFERENCES public.payroll_run_items(id) ON DELETE CASCADE;
...
--
-- Name: work_code_category_mappings fk_wccm_mapping_type; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_mappings
    ADD CONSTRAINT fk_wccm_mapping_type FOREIGN KEY (mapping_type) REFERENCES public.work_code_category_mapping_types(code) ON UPDATE CASCADE ON DELETE RESTRICT;


--
-- Name: work_code_category_scheme_rules fk_wccsr_effective_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_scheme_rules
    ADD CONSTRAINT fk_wccsr_effective_category FOREIGN KEY (effective_category_id) REFERENCES public.work_code_categories(id) ON DELETE RESTRICT;


--
-- Name: work_code_category_scheme_rules fk_wccsr_scheme; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_scheme_rules
    ADD CONSTRAINT fk_wccsr_scheme FOREIGN KEY (compensation_scheme_id) REFERENCES public.compensation_schemes(id) ON DELETE RESTRICT;


--
-- Name: work_code_category_scheme_rules fk_wccsr_source_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_scheme_rules
    ADD CONSTRAINT fk_wccsr_source_category FOREIGN KEY (source_category_id) REFERENCES public.work_code_categories(id) ON DELETE RESTRICT;


--
-- Name: work_code_category_translations fk_wcct_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_code_category_translations
    ADD CONSTRAINT fk_wcct_category FOREIGN KEY (work_code_category_id) REFERENCES public.work_code_categories(id) ON DELETE CASCADE;
...
--
-- Name: work_logs fk_work_logs_compensation_scheme; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_logs
    ADD CONSTRAINT fk_work_logs_compensation_scheme FOREIGN KEY (compensation_scheme_id) REFERENCES public.compensation_schemes(id) ON DELETE RESTRICT;
...
--
-- Name: work_logs fk_work_logs_scheme_effective_category; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_logs
    ADD CONSTRAINT fk_work_logs_scheme_effective_category FOREIGN KEY (scheme_effective_work_code_category_id) REFERENCES public.work_code_categories(id) ON DELETE RESTRICT;


--
-- Name: work_logs fk_work_logs_scheme_rule; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_logs
    ADD CONSTRAINT fk_work_logs_scheme_rule FOREIGN KEY (work_code_category_scheme_rule_id) REFERENCES public.work_code_category_scheme_rules(id) ON DELETE RESTRICT;
...
--
-- Name: operation_norm_versions operation_norm_versions_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.operation_norm_versions
    ADD CONSTRAINT operation_norm_versions_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE RESTRICT;


--
-- Name: operation_norm_versions operation_norm_versions_operation_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.operation_norm_versions
    ADD CONSTRAINT operation_norm_versions_operation_id_fkey FOREIGN KEY (operation_id) REFERENCES public.operations(id) ON DELETE CASCADE;


--
-- Name: operation_norm_versions operation_norm_versions_verified_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.operation_norm_versions
    ADD CONSTRAINT operation_norm_versions_verified_by_fkey FOREIGN KEY (verified_by) REFERENCES public.users(id) ON DELETE RESTRICT;


--
-- Name: payroll_field_access payroll_field_access_updated_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_field_access
    ADD CONSTRAINT payroll_field_access_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id);


--
-- Name: payroll_run_item_handovers payroll_run_item_handovers_actor_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_run_item_handovers
    ADD CONSTRAINT payroll_run_item_handovers_actor_id_fkey FOREIGN KEY (actor_id) REFERENCES public.users(id);


--
-- Name: payroll_run_item_handovers payroll_run_item_handovers_payroll_run_item_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payroll_run_item_handovers
    ADD CONSTRAINT payroll_run_item_handovers_payroll_run_item_id_fkey FOREIGN KEY (payroll_run_item_id) REFERENCES public.payroll_run_items(id) ON DELETE CASCADE;
...
--
-- Name: work_shifts work_shifts_archived_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.work_shifts
    ADD CONSTRAINT work_shifts_archived_by_fkey FOREIGN KEY (archived_by) REFERENCES public.users(id);

package com.aleksandarparipovic.marel_app.sample_order.specification;
...
/**
 * How the sample-order screens narrow the table.
 *
 * <p>The production-order counterpart with the parts samples do not have taken
 * out — there are no priority flags to filter on, and a line has one quantity
 * rather than a series of dated ones. Everything else is deliberately the same
 * rule, including the LIKE escaping, because the two screens are searched by the
 * same people expecting the same behaviour.
 */
public final class SampleOrderSpecifications {

    private static final SampleOrderFieldMapper FIELD_MAPPER = new SampleOrderFieldMapper();
     ...
     * that decides WHICH line matched reads the typed text literally.
     */
    private static final char LIKE_ESCAPE = '\\';

    private SampleOrderSpecifications() {
    }

    public static Specification<SampleOrder> fromSearchRequest(SearchRequest request) {
        return Specification.where(notArchived())
                .and(new SearchSpecification<>(request, FIELD_MAPPER));
    }

    public static Specification<SampleOrder> notArchived() {
     ...
     *
     * <p>Whether an order matched at all is decided HERE; WHICH line matched is
     * decided in the service, on the same lower-case contains rule. The two must
     * agree, which is why the pattern below is escaped.
     */
    public static Specification<SampleOrder> matchesText(String text) {
        String pattern = "%" + escapeLike(text.toLowerCase(Locale.ROOT)) + "%";

        return (root, query, cb) -> {
            Subquery<Integer> lineItemMatch = query.subquery(Integer.class);
            Root<SampleOrderLineItem> lineItem = lineItemMatch.from(SampleOrderLineItem.class);
            var product = lineItem.get("product");

            Subquery<Integer> lineItemNoteMatch = lineItemMatch.subquery(Integer.class);
            Root<SampleOrderLineItemNote> lineItemNote = lineItemNoteMatch.from(SampleOrderLineItemNote.class);
            lineItemNoteMatch.select(cb.literal(1)).where(
                    cb.equal(lineItemNote.get("sampleOrderLineItem"), lineItem),
                    cb.isTrue(lineItemNote.get("isActive")),
                    like(cb, lineItemNote.get("note"), pattern)
            );

            lineItemMatch.select(cb.literal(1)).where(
                    cb.equal(lineItem.get("sampleOrder"), root),
     ...
     * floor, and the quantity read as text.
     *
     ...
     */
    public static Specification<SampleOrder> matchesAnything(String text) {
        String pattern = "%" + escapeLike(text.toLowerCase(Locale.ROOT)) + "%";

        Specification<SampleOrder> extra = (root, query, cb) -> {
    ...
    public static Specification<SampleOrder> customerIs(Long customerId) {
        return forCustomer(customerId);
    }

    /** Orders written by one person. */
    public static Specification<SampleOrder> writtenBy(Long userId) {
     ...
     *
     * <p>Either end may be null for an open span. The two are swapped when they
     * arrive the wrong way round, because a reversed range is a slip rather than
     * a request for nothing.
     */
    public static Specification<SampleOrder> createdBetween(LocalDate from, LocalDate to) {

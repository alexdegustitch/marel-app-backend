...
/**
 * One address a sample order is (or was) sent to.
 *
 ...
 *
 * <p>Its own table rather than a shared one with the production-order
 * recipients: a recipient belongs to exactly one kind of order, and a shared
 * table would need a CHECK to say so plus every query to remember it. The
 * {@link RecipientSourceType} enum IS shared, because "where did this address
 * come from" has the same three answers on both.
 */
@Entity
@Table(name = "sample_order_recipients")
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class SampleOrderRecipient {

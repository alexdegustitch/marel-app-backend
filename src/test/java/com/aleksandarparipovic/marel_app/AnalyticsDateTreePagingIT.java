...
class AnalyticsDateTreePagingIT extends AbstractIntegrationTest {
    ...
    @Test
    @DisplayName("the operation report is a paged LIST of operations, ranked on the server")
    void operationReportIsAPagedList() {
        Fixture data = seedThreeDaysTwoShiftsTwoOperations();
        ...
        AnalyticsPageDto<OperationSummaryDto> first =
                analyticsQueryRepository.findOperationEfficiencyPage(filter);

        // Two operations were worked: one row each, and one row per page. No worker grain —
        // the same operation worked by three people is still one row.
        assertThat(first.page().totalElements()).isEqualTo(2);
        assertThat(first.content()).hasSize(1);

        // Ranked across the whole period rather than within the chunk: B produced 681 to A's
        // 675, so B is what the first page holds.
        OperationSummaryDto top = first.content().getFirst();
        assertThat(top.getOperationId()).isEqualTo(data.operationBId);
        assertThat(top.getSumQuantity()).isEqualTo(681);
        // The product rides along as context — an operation has exactly one.
        assertThat(top.getProductId()).isEqualTo(data.productId);
    }
